...
// 订阅指定主题，并指定回调函数，1000为队列大小，当我们来不及处理消息时，会存储在该队列中，若队列元素大于1000，则会抛弃老的消息
  ...
  odom_sub_ = nh.subscribe("/odom_world", 1, &KinoReplanFSM::odometryCallback, this); //话题已重映射
...
//获取目标点，设置目标位置、速度、允许最大速度和加速度
void KinoReplanFSM::waypointCallback(const nav_msgs::PathConstPtr& msg) 
{
  // if (msg->poses[0].pose.position.z < -0.1) return;

  cout << "Triggered!" << endl;
  trigger_ = true; have_target_ = true;

  end_pt_ << msg->poses[0].pose.position.x, msg->poses[0].pose.position.y, msg->poses[0].pose.position.z;
  end_vel_.setZero();
  mission_max_vel_ = msg->poses[1].pose.position.x;
  mission_max_acc_ = msg->poses[1].pose.position.y;

  cout << "【规划】目标点已经获取："<< end_pt_[0] << " [m] "<<  end_pt_[1]  << " [m] "<<  end_pt_[2]  << " [m] "<<endl;
  cout << "【规划】最大速度和加速度："<< mission_max_vel_ << " [m/s] "
                              << mission_max_acc_  << " [m/s^2] " <<endl;
 ...
 {  
  ...
  switch(exec_state_) 
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      Eigen::Vector3d LocalTraj_goal = info->position_traj_.evaluateDeBoorT(info->duration_-0.1); //这段轨迹的终点

      /* && t_cur > info->duration_ - 1e-2*/
      if ((end_pt_ - pos).norm() < 0.3)
      {
        have_target_ = false;
        changeFSMExecState(WAIT_TARGET, "FSM");
        return;
      } 
      else if ((LocalTraj_goal - end_pt_).norm() < no_replan_thresh_) // 当前轨迹的终点（算出来） 是否已抵达规划终点附近end_pt_
      {
        if(fsm_num2 > 100)
        {
          fsm_num2 = 0;
          cout << "当前轨迹的终点靠近规划终点，不再重规划" << endl;
        }
        return;
      }
...
bool KinoReplanFSM::callKinodynamicReplan() {
  bool plan_success =
      planner_manager_->kinodynamicReplan(start_pt_, start_vel_, start_acc_, 
                                                    end_pt_, end_vel_, mission_max_vel_, mission_max_acc_);
