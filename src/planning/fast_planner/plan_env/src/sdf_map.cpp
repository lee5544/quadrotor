...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  node_.param("sdf_map/pose_type", mp_.pose_type_, 3);
  ...
  // md_.proj_points_.resize(640 * 480 / mp_.skip_pixel_);
  md_.proj_points_cnt = 0;

  /* init callback */
  //订阅深度图和里程计geometry_msgs::PoseStamped
  if (mp_.pose_type_ == POSE_DEPTH) {
    std::cout<<"USE pose and depth image"<<std::endl;
  ...
  //订阅深度图和里程计nav_msgs::Odometry，将sdf_map/pose_type设置成2
  else if (mp_.pose_type_ == ODOMETRY_DEPTH) {
    std::cout<<"USE odometry and depth image"<<std::endl;
  ...
  //订阅点云和里程计use odometry and point cloud
  else if (mp_.pose_type_ == ODOMETRY_POINTS){
    std::cout<<"USE odometry and poinds"<<std::endl;
    odom_sub_.reset(new message_filters::Subscriber<nav_msgs::Odometry>(node_, "/sdf_map/odom", 100));
    poinds_sub_.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(node_, "/sdf_map/cloud", 10));

    sync_poinds_odom_.reset(new message_filters::Synchronizer<SyncPolicyPoindsOdom>(
        SyncPolicyPoindsOdom(100),*poinds_sub_, *odom_sub_));
    sync_poinds_odom_->registerCallback(boost::bind(&SDFMap::poindsOdomCallback, this, _1, _2));
...
void SDFMap::projectDepthImage() {
  ...
  // camera_r.transpose();//更改

  Eigen::Matrix3d camera_r_dot = Eigen::Matrix3d::Zero();
  camera_r_dot << 0,0,1,   -1,0,0,   0,-1,0;
  camera_r = camera_r * camera_r_dot;

  // camera_r_dot << 1,0,0,   0,1,0,   0,0,1;
  // camera_r = camera_r * camera_r_dot;
...
void SDFMap::clearAndInflateLocalMap() {
  ...
  int inf_step =2;// ceil(mp_.obstacles_inflation_ / mp_.resolution_);
  // int inf_step_z = 1;
...
void SDFMap::updateESDFCallback(const ros::TimerEvent& /*event*/) {
  ...
  t2 = ros::Time::now();
  // std::cout << "update ESDF time: "  << t2.toSec() - t1.toSec() <<std::endl;
...
void SDFMap::poindsOdomCallback(const sensor_msgs::PointCloud2ConstPtr& img, 
                                const nav_msgs::OdometryConstPtr& odom)
{
  ...
  for (size_t i = 0; i < latest_cloud.points.size(); ++i) {
    pt_origin = latest_cloud.points[i];//点云在其测量坐标系中的坐标
    pt  = pt_origin;
    // pt.x = pt_origin.x, pt.y = pt_origin.z, pt.z = -pt_origin.y; //将点云坐标系转换成载体坐标系
    
    p3d(0) = pt.x, p3d(1) = pt.y, p3d(2) = pt.z;//将点云载体坐标系转换成全局坐标系(右前上)中
    p3d = md_.camera_q_ * p3d  + md_.camera_pos_;
    ...
    if (fabs(pt.x) < mp_.local_update_range_(0) && fabs(pt.y) < mp_.local_update_range_(1) &&
        fabs(pt.z) < mp_.local_update_range_(2))//保证点云在局部地图范围内部
    {
      ...
      for (int x = -inf_step; x <= inf_step; ++x)
        for (int y = -inf_step; y <= inf_step; ++y)
          for (int z = -inf_step_z; z <= inf_step_z; ++z) {
            ...
            posToIndex(p3d_inf, inf_pt);//找到该点在大地图中的位置
