...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  ...
  if (use_kinodynamic_path) {
    ...
    kino_path_finder_->setParam(nh);                                                    //设置参数，前端最大加速度、速度……
...
bool FastPlannerManager::kinodynamicReplan(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
                                           ...
                                           double mission_max_vel, double mission_max_acc) {

  std::cout << "\n[kino replan]: -----------------------" << std::endl;
  cout << "起点start: " << start_pt.transpose() << ", " << start_vel.transpose() << ", "
       << start_acc.transpose() << "\n终点goal:" << end_pt.transpose() << ", " << end_vel.transpose()
  ...
  ctrl_pts = bspline_optimizers_[0]->BsplineOptimizeTraj(ctrl_pts, ts, cost_function, 1, 1);  //soft 
  ...
  // pos.setPhysicalLimits(pp_.max_vel_, pp_.max_acc_); //在这里添加动力学约束！！
  pos.setPhysicalLimits(mission_max_vel, mission_max_acc); //modify
  ...
  // pos.checkFeasibility(true);
   cout << "后段优化时间重分配次数 [Main]: iter num: " << iter_num << endl;

  double tn = pos.getTimeSum();

  cout << "后端优化时间重分配的次数 [kino replan]: Reallocate ratio: " << tn / to << endl;
...
void FastPlannerManager::refineTraj(NonUniformBspline& best_traj, double& time_inc) {
  ...
  best_traj.setPhysicalLimits(pp_.max_vel_, pp_.max_acc_);//设置最大速度和加速度
...
void FastPlannerManager::planYaw(const Eigen::Vector3d& start_yaw) 
{
  ...
  Eigen::Vector3d end_v = local_data_.velocity_traj_.evaluateDeBoorT(duration - dt_yaw);
