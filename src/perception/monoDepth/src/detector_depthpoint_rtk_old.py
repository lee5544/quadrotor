...
class Mono_detector():
    def __init__(self):
        ...
        self.estimator = DBSCAN(eps=1, min_samples=15, metric='euclidean')
    ...
    def multi_callback(self, odom_msg, img_msg):
        ...
        if self.keyframe == False and self.init == True:
            ...
            if len(self.data_buff) >= buff_num:
                ...
                ce = self.MyDBSCAN(np.array(data))
                # print(ce)
                # print('*******************************************')
                self.data_buff.pop(0)            
                ce_veh =   convert.Convert().world2uav(ce,R, T)
                ce_veh_avg = np.sum(ce_veh, axis=0)/3 if ce_veh!= [] else [0]
                if np.abs(ce_veh_avg[0])> 7 and np.abs(ce_veh_avg[0]) < 9 and len(ce)==3:
    ...
    def vision_detector(self, image):
        ...
        for n in range(0, cnt_num):
            ...
            cv2.putText(image, str(round(dist, 2))+'m', (int(rect[0][0]), int(
                rect[0][1])), cv2.FONT_HERSHEY_PLAIN, 1.0, (0, 0, 255), 2)
